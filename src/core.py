...
def generate_wide_data(stores: list | None=None, months: list | None=None) -> pd.DataFrame:
...
def wide_to_long(df: pd.DataFrame, id_vars: str, var_name: str='Month', value_name: str='Sales', suffix: str='_Sales') -> pd.DataFrame:
...
def pivot_table_aggregation(df: pd.DataFrame, index: str, columns: str, values: str, aggfunc: str='sum') -> pd.DataFrame:
...
def groupby_aggregation(df: pd.DataFrame, groupby_cols: list, value_col: str, aggfuncs: dict[str, str]) -> pd.DataFrame:
    """Perform groupby aggregation with multiple functions."""
    return df.groupby(groupby_cols)[value_col].agg(aggfuncs).reset_index()

def generate_weekly_sales_data(stores: list | None=None, weeks: int=3) -> pd.DataFrame:
...
def plot_store_comparison(df: pd.DataFrame, output_path: Path, plot: bool=False):
    ...
    for i, store in enumerate(pivot.columns[1:]):
        ax.plot(pivot['Week'], pivot[store], label=store, color=colors[i % len(colors)], linewidth=1.2, marker='o')
    ax.set_xlabel('Week')
    ax.set_ylabel('Sales')
    ax.legend(loc='best')
    plt.savefig(output_path, dpi=100, bbox_inches='tight')
